...
public static class SeedingLocalExtension
{
    ...
    public static async Task SeedDataAsync(this WebApplication app)
    {
        ...
        _dbContext = scope.ServiceProvider.GetRequiredService<StreetcodeDbContext>();
        ...
        await SeedPositions();
        await SeedTeamMemberPositions();
        await SeedTeamMembersLinks();
        await SeedStreetcodes();

        await _dbContext.SaveChangesAsync();
    }
    
    private static void CreateBlobStorageFolder(WebApplication app)
    {
        _blobPath = app.Configuration.GetValue<string>("Blob:BlobStorePath")
    ...
    private static async Task SeedImageDetails()
    {
        if (_dbContext.ImageDetailses!.Any()) return;
        
        _dbContext.ImageDetailses!.AddRange(new[]
        {
            new ImageDetails()
            {
                ImageId = 6,
    ...
    private static async Task SeedResponses()
    {
        if (_dbContext.Responses!.Any()) return;
        
        _dbContext.Responses!.AddRange(
            new Response
            {
                Name = "Alex",
    ...
    private static async Task SeedTerms()
    {
        if (_dbContext.Terms!.Any()) return;
        ...
        _dbContext.Terms!.AddRange(
            new Term
            {
                Title = "етнограф",
                Description =
                    "Етнографія — суспільствознавча наука, об'єктом дослідження якої є народи, їхня культура і побут, походження, розселення," +
            ...
            new Term
            {
                Title = "гравер",
                Description =
                    "Гра́фіка — вид образотворчого мистецтва, для якого характерна перевага ліній і штрихів, використання контрастів білого та" +
            ...
            new Term
            {
                Title = "кріпак",
                Description =
                    "Кріпа́цтво, або кріпосне́ право, у вузькому сенсі — правова система, або система правових норм при феодалізмі, яка встановлювала" +
        ...
        await SeedRelatedTerms();
    }

    private static async Task SeedRelatedTerms()
    {
        if (_dbContext.RelatedTerms!.Any()) return;
        
        _dbContext.RelatedTerms!.AddRange(
            new RelatedTerm
            {
                Word = "кріпаків",
                TermId = 3,
    ...
    private static async Task SeedTeamMembers()
    {
        if (_dbContext.TeamMembers!.Any()) return;
    ...
    private static async Task SeedPositions()
    {
        if (_dbContext.Positions!.Any()) return;
        
        _dbContext.Positions!.AddRange(
            new Positions
            {
    ...
    private static async Task SeedTeamMemberPositions()
    {
        if (_dbContext.TeamMemberPosition!.Any()) return;
        
        _dbContext.TeamMemberPosition!.AddRange(
    ...
    private static async Task SeedTeamMembersLinks()
    {
        const string targetUrl = "https://www.youtube.com/watch?v=8kCnOqvmEp0&ab_channel=JL%7C%D0%AE%D0%9B%D0%86%D0%AF%D0%9B%D0%A3%D0%A9%D0%98%D0%9D%D0%A1%D0%AC%D0%9A%D0%90";
        
        if (_dbContext.TeamMemberLinks!.Any()) return;
    ...
    private static async Task SeedNews()
    {
        if (_dbContext.News!.Any()) return;
        
        _dbContext.News!.AddRange(
    ...
    private static async Task SeedStreetcodes()
    {
        if (_dbContext.Streetcodes!.Any()) return;
        
        _dbContext.Streetcodes!.AddRange(
            new PersonStreetcode
            {
                Index = 1,
                TransliterationUrl = "taras-shevchenko",
                Teaser =
                    "Тара́с Григо́рович Шевче́нко (25 лютого (9 березня) 1814, с. Моринці, Київська губернія," +
        ...
        await SeedTransactionLinks();
        await SeedComments();
        await SeedFacts();
        await SeedSourceLinkCategories();
        await SeedRelatedFigures();
        await SeedStreetcodeImages();
        await SeedTags();
    }

    private static async Task SeedSubtitles()
    {
        if (_dbContext.Subtitles!.Any()) return;
        
        _dbContext.Subtitles!.AddRange(
            new Subtitle
            {
    ...
    private static async Task SeedCoordinates()
    {
        if (_dbContext.StreetcodeCoordinates!.Any()) return;
        
        _dbContext.StreetcodeCoordinates!.AddRange(
    ...
    private static async Task SeedVideos()
    {
        if (_dbContext.Videos!.Any()) return;
    ...
    private static async Task SeedPartners()
    {
        if (_dbContext.Partners!.Any()) return;
        ...
        await SeedPartnersSourceLinks();
        await SeedStreetcodePartners();
    }

    private static async Task SeedPartnersSourceLinks()
    {
        if (_dbContext.PartnerSourceLinks!.Any()) return;
        
        _dbContext.PartnerSourceLinks!.AddRange(
    ...
    private static async Task SeedStreetcodePartners()
    {
        if (_dbContext.StreetcodePartners!.Any()) return;
        
        _dbContext.StreetcodePartners!.AddRange(
    ...
    private static async Task SeedArts()
    {
        if (_dbContext.Arts!.Any()) return;
        
        _dbContext.Arts!.AddRange(
            new Art
            {
                ImageId = 19,
        ...
        await SeedStreetcodeArts();
    }

    private static async Task SeedStreetcodeArts()
    {
        if (_dbContext.StreetcodeArts!.Any()) return;
        
        _dbContext.StreetcodeArts!.AddRange(
            new StreetcodeArt
            {
                ArtId = 1,
                StreetcodeId = 1,
    ...
    private static async Task SeedTexts()
    {
        if (_dbContext.Texts!.Any()) return;
        
        _dbContext.Texts!.AddRange(
            new Text
            {
    ...
    private static async Task SeedTimelineItems()
    {
        if (!_dbContext.TimelineItems!.Any()) return;
        
        _dbContext.TimelineItems!.AddRange(
            new TimelineItem
            {
        ...
        await SeedHistoricalContexts();
    }

    private static async Task SeedHistoricalContexts()
    {
        if (_dbContext.HistoricalContexts!.Any()) return;
        
        _dbContext.HistoricalContexts!.AddRange(
        ...
        await SeedHistoricalContextTimelines();
    }

    private static async Task SeedHistoricalContextTimelines()
    {
        if (_dbContext.HistoricalContextsTimelines!.Any()) return;
        
        _dbContext.HistoricalContextsTimelines!.AddRange(
    ...
    private static async Task SeedTransactionLinks()
    {
        if (_dbContext.TransactionLinks!.Any()) return;
        
        _dbContext.TransactionLinks!.AddRange(
            new TransactionLink
            {
    ...
    private static async Task SeedComments()
    {
        if (_dbContext.Comments!.Any()) return;
    ...
    private static async Task SeedFacts()
    {
        if (_dbContext.Facts!.Any()) return;
        
        _dbContext.Facts!.AddRange(
            new Fact
            {
    ...
    private static async Task SeedSourceLinkCategories()
    {
        if (_dbContext.SourceLinks!.Any()) return;
        
        _dbContext.SourceLinks!.AddRange(
        ...
        await SeedSourceLinkCategoryContent();
    }

    private static async Task SeedSourceLinkCategoryContent()
    {
        if (_dbContext.StreetcodeCategoryContent!.Any()) return;
        
        _dbContext.StreetcodeCategoryContent!.AddRange(
    ...
    private static async Task SeedRelatedFigures()
    {
        if (_dbContext.RelatedFigures!.Any()) return;
        
        _dbContext.RelatedFigures!.AddRange(
            new RelatedFigure
            {
                ObserverId = 2,
                TargetId = 1
    ...
    private static async Task SeedStreetcodeImages()
    {
        if (_dbContext.StreetcodeImages!.Any()) return;
        
        _dbContext.StreetcodeImages!.AddRange(
            new StreetcodeImage
            {
                ImageId = 1,
                StreetcodeId = 1,
    ...
    private static async Task SeedTags()
    {
        ...
        await SeedStreetcodeTagIndices();
    }

    private static async Task SeedStreetcodeTagIndices()
    {
        if (!_dbContext.StreetcodeTagIndices!.Any()) return;
        
        _dbContext.StreetcodeTagIndices!.AddRange(
